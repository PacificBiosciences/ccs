...
P6C4NoCovModel::Populate(const std::string& tpl) const
{
    ...
    {
        const uint8_t b = TranslationTable[static_cast<uint8_t>(tpl[i])];

        if (b > 3) throw std::invalid_argument("invalid character in sequence!");

        const bool hp = tpl[i - 1] != tpl[i]; // e.g. AA = 0, NA = 1
        const auto params = NoCovP4C4Params[b][hp];
        const double snr = snr[b], snr2 = snr * snr, snr3 = snr2 * snr;
        double tprobs[3];

        for (size_t j = 0; j < 3; ++j)
        {
            double xb =        params[j][0]
                      + snr  * params[j][1]
                      + snr2 * params[j][2]
                      + snr3 * params[j][3];
            xb = std::exp(xb);
...
double P6C4NoCovModel::CovariatePr(MoveType move, uint8_t cov) const
{
    if (move == DELETION)
        throw std::argument_error("invalid move! DELETION has no covariate!");

    return 1.0;
}

double P6C4NoCovModel::MiscallPr(char from, char to) const
{
    return 0.00505052456472967;
}

void P6C4NoCovModel::Register()
{
    auto tbl = ParameterTable::Default_();

    tbl.tbl_["P6C4.NoCov"] = [] (const SNR& snr) { return std::unique_ptr<ModelConfig>(new P6C4NoCovModel(snr)); };
}

P6C4NoCovModel::Register();

bool ParameterTable::Contains(const std::string& name) const
{
    return tbl_.find(model) != tbl_.end();
}

const ModelConfig& ParameterTable::At(const std::string& name) const
{
    return *tbl_.at(model);
}
