...
size_t Template::Length() const
{
    return tpl_.size() + mutOff_;
}

const TemplatePosition& Template::operator[](size_t i) const
{
    // if no mutation, or everything up to the base before mutPos_, just return what we have
    if (!IsMutated() || i + 1 < mutPos_)
        return tpl_[i];
    ...
    // otherwise if we're the base before mutPos_, 0, else 1 of our mutated tpl params
    return mutTpl_[i == mutPos_];
}

bool Template::IsMutated() const
{
    return mutated_;
}
...
void Template::Mutate(const Mutation& mut)
{
    ...
    {
        if (mutPos_ > 0)
            mutTpl_[0] = cfg_->Populate({ tpl_[mutPos_ - 1].Base, mut.Base })[0];
    ...
    else if (mut.Type == MutationType::DELETION)
    {
        ...
        if (mutPos_ + 1 < tpl_.size())
            mutTpl_[1] = tpl_[mutPos_ + 1];
    }
    else
        throw std::invalid_argument("invalid mutation type! must be DELETION, INSERTION, or SUBSTITUTION");

    mutOff_ = mut.LengthDiff();
    mutated_ = true;

    assert((*this)[Length() - 1].Match    == 0.0 &&
           (*this)[Length() - 1].Branch   == 0.0 &&
           (*this)[Length() - 1].Stick    == 0.0 &&
           (*this)[Length() - 1].Deletion == 0.0);
}
...
void Template::ApplyMutation(const Mutation& mut)
{
    ...
    {
        if (i > 0)
            tpl_[i - 1] = cfg_->Populate({ tpl_[i - 1].Base, mut.Base })[0];
